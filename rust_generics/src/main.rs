...
fn main() {
    ...
    println!("integer_plus_float variants {},{}", integer_plus_float.x,integer_plus_float.y);


    let tuple = Tuple{first:1, second:4};
    println!("sum of variants in tuple {}", sum(tuple));

    let two_tuple : traits::TwoTuple<u32> = traits::TwoTuple{first:1u32, second:4u32,};
    let three_tuple : traits::ThreeTuple<u32> = traits::ThreeTuple{first: 2u32, second : 3u32, third: 5u32};
